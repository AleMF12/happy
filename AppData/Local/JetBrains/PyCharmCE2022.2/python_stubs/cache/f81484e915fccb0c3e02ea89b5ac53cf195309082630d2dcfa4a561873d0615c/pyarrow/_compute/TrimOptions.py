# encoding: utf-8
# module pyarrow._compute
# from C:\Users\#Ale\.conda\envs\GCS10004\lib\site-packages\pyarrow\_compute.cp39-win_amd64.pyd
# by generator 1.147
# no doc

# imports
import builtins as __builtins__ # <module 'builtins' (built-in)>
import sys as sys # <module 'sys' (built-in)>
import pyarrow.lib as lib # C:\Users\#Ale\.conda\envs\GCS10004\lib\site-packages\pyarrow\lib.cp39-win_amd64.pyd
import inspect as inspect # C:\Users\#Ale\.conda\envs\GCS10004\lib\inspect.py
import numpy as np # C:\Users\#Ale\.conda\envs\GCS10004\lib\site-packages\numpy\__init__.py
from pyarrow.lib import ArrowInvalid, frombytes, tobytes

import pyarrow.lib as __pyarrow_lib


from ._TrimOptions import _TrimOptions

class TrimOptions(_TrimOptions):
    """
    Options for trimming characters from strings.
    
        Parameters
        ----------
        characters : str
            Individual characters to be trimmed from the string.
    """
    def __init__(self, characters): # real signature unknown; restored from __doc__
        """ TrimOptions.__init__(self, characters) """
        pass

    __dict__ = None # (!) real value is "mappingproxy({'__module__': 'pyarrow._compute', '__doc__': '\\n    Options for trimming characters from strings.\\n\\n    Parameters\\n    ----------\\n    characters : str\\n        Individual characters to be trimmed from the string.\\n    ', '__init__': <cyfunction TrimOptions.__init__ at 0x000002BD21D3B110>, '__dict__': <attribute '__dict__' of 'TrimOptions' objects>})"


