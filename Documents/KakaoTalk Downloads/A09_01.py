...
for j in range(66):
    if book_abbre[j][-1] == '1':
        if book_abbre[j].split(" ")[1:4][0][:2]+book_abbre[j].split(" ")[0] == book_full[j]:
            full2abbrev[book_abbre[j]] = book_full[j]
    elif book_abbre[j][-1] == '2':
        if book_abbre[j].split(" ")[1:4][0][:2]+book_abbre[j][j].split(" ")[0] == book_full[j]:
            full2abbrev[book_abbre[j]] = book_full[j]
    elif book_abbre[j][-1] == '3':
        if book_abbre[j].split(" ")[1:4][0][:2]+book_abbre[j].split(" ")[0] == book_full[j]:
            full2abbrev[book_abbre[j]] = book_full[j]
    else:
        if book_abbre[j].split(" ")[0][:3] == book_full[j]:
            full2abbrev[book_abbre[j]] = book_full[j]


# print(len(full2abbrev))
# print(len(book_full))
with open(f"bible-fullname.txt", "rt") as fin:
    full_bible = fin.read()
