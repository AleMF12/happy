...
class Bible:
    ...
    def _load_book_names(self):
        ...
        for j in range(66):
            if self.book_abbr_names[j][-1] == '1':
                self.abbrev2full[self.book_abbr_names[j]] = self.book_full_names[j]

            elif self.book_abbr_names[j][-1] == '2':
                self.abbrev2full[self.book_abbr_names[j]] = self.book_full_names[j]

            elif self.book_abbr_names[j][-1] == '3':
                self.abbrev2full[self.book_abbr_names[j]] = self.book_full_names[j]
        ...
        for i in range(66):
            if self.book_full_names[i].split(" ")[0] == '1':
                self.full2abbrev[self.book_full_names[i]] = self.book_abbr_names[i]
            elif self.book_full_names[i].split(" ")[0] == '2':
                self.full2abbrev[self.book_full_names[i]] = self.book_abbr_names[i]
            elif self.book_full_names[i].split(" ")[0] == '3':
                self.full2abbrev[self.book_full_names[i]] = self.book_abbr_names[i]
            else:
                self.full2abbrev[self.book_full_names[i]] = self.book_abbr_names[i]
    ...
    def _load_bible(self):
        ...
        for abbr in self.book_abbr_names:
            ...
            ### YOUR CODE STARTS HERE
            with open(f"{abbr}.txt", "rt") as fin:
                line = fin.read()
    ...
    def search(self, book_full_name, chapter_num, verse_num):
        ...
        ### YOUR CODE STARTS HERE
        value = (book_full_name, chapter_num, verse_num)
    ...
    def search_by_keyword(self, keyword):
        ...
        ### YOUR CODE STARTS HERE
        if keyword in self.bible:
            found.append(self.bible[keyword])
